...
export default async function PurchaseOrdersPage({ searchParams }: PageProps) {
  ...
  const where = {
    ...
    ...(statusFilter && validStatuses.includes(statusFilter)
      ? { status: statusFilter as "DRAFT" | "SENT" | "PARTIAL" | "RECEIVED" | "CANCELLED" }
      : {}),
  };
    ...
    <div style={{ minHeight: "100vh", background: "#0b1326", padding: "24px" }}>
      <div style={{ maxWidth: "1200px", margin: "0 auto" }}>
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "13px" }}>
              <thead>
                <tr style={{ borderBottom: "1px solid #222a3e", background: "#0d1627" }}>
                  {["PO ID", "Supplier", "Expected Delivery", "Total Value", "Status", "Created", "Actions"].map((h) => (
                    <th
                      key={h}
                      style={{
                        padding: "12px 16px",
                ...
                {orders.length === 0 ? (
                  <tr>
                    <td colSpan={7} style={{ padding: "48px 24px", textAlign: "center", color: "#8c90a2", fontSize: "14px" }}>
                      {statusFilter ? `No ${statusFilter.toLowerCase()} purchase orders found.` : "No purchase orders yet."}
                    </td>
                  </tr>
                ) : (
                  orders.map((po) => {
                    ...
                    return (
                      <tr key={po.id} className="po-row" style={{ borderBottom: "1px solid #1a2237" }}>
        ...
        <div style={{ marginTop: "12px", fontSize: "12px", color: "#4a5068", textAlign: "right" }}>
          {total} purchase order{total !== 1 ? "s" : ""} total
        </div>
      </div>
    </div>
  );
