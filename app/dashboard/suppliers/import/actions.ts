...
export const SUPPLIER_IMPORT_COLUMNS: ImportColumnDef[] = [
  { key: "name", label: "Name", required: true, aliases: ["suppliername", "company", "companyname"] },
...
function toRowInput(values: Record<string, string>): SupplierRowInput {
  return {
    name: values.name ?? "",
    email: values.email ?? "",
    phone: values.phone ?? "",
...
export async function validateSupplierImportAction(
...
export async function commitSupplierImportAction(
  ...
  await writeAuditLog({
    ...
    action: "settings.import.suppliers",
    entityType: "Supplier",
