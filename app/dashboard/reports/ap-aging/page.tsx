...
export default async function ApAgingReportPage({
  ...
  await requirePagePermission(session, "reports.ap.view");

  const params = await searchParams;
  const view = params.view === "invoice" ? "invoice" : "supplier";
  ...
  // Suppliers scoped to this org only (for the filter dropdown).
  ...
  const effectiveSupplierId =
    supplierIdFilter && validSupplierIds.has(supplierIdFilter)
      ? supplierIdFilter
      : "";

  // Confirmed purchase invoices, scoped to this warehouse session, with payments.
  ...
  const suppliersWithBalance = supplierRows.length;

  // Build export URL preserving the current filters and active view.
  const exportParams = new URLSearchParams();
  exportParams.set("view", view);
  if (effectiveSupplierId) exportParams.set("supplierId", effectiveSupplierId);
  exportParams.set("asOf", asOfStr);
  const exportUrl = `/dashboard/reports/ap-aging/export?${exportParams.toString()}`;

  function viewUrl(targetView: "supplier" | "invoice"): string {
    const p = new URLSearchParams();
    p.set("view", targetView);
    if (effectiveSupplierId) p.set("supplierId", effectiveSupplierId);
    p.set("asOf", asOfStr);
    return `/dashboard/reports/ap-aging?${p.toString()}`;
  }
      ...
      <div style={{ maxWidth: "1200px", margin: "0 auto" }}>
        ...
        >
          <div>
            ...
            <p style={{ fontSize: "13px", color: "#8c90a2", marginTop: "4px" }}>
              Outstanding purchase invoices grouped by how many days overdue, as
              of {fmtDate(asOfDate)}.
            </p>
          </div>
          <Link
        ...
        >
          <StatCard
            ...
            sub={effectiveSupplierId ? "Selected supplier" : "All suppliers"}
            prominent
          />
          <StatCard
        ...
        >
              ...
              style={{
                ...
                border:
                  view === "supplier"
                    ...
                    : "1px solid #2d3449",
                background:
                  view === "supplier" ? "rgba(0,98,255,0.12)" : "#0d1627",
                color: view === "supplier" ? "#7da6ff" : "#8c90a2",
                fontSize: "13px",
                fontWeight: 600,
                textDecoration: "none",
              }}
          ...
          >
            ...
            <select
              name="supplierId"
              defaultValue={effectiveSupplierId}
              style={{
                padding: "8px 12px",
                background: "#0d1627",
                border: "1px solid #2d3449",
                borderRadius: "8px",
                color: effectiveSupplierId ? "#dbe2fd" : "#4a5068",
                fontSize: "13px",
                outline: "none",
                minWidth: "200px",
              }}
            ...
            {(effectiveSupplierId || asOfStr !== todayStr) && (
              <Link
                href={`/dashboard/reports/ap-aging?view=${view}`}
                style={{
                  padding: "8px 12px",
                  border: "1px solid #2d3449",
                  borderRadius: "8px",
                ...
                {supplierRows.length > 0 && (
                  <tfoot>
                    ...
                    >
                      ...
                      >
                        {fmtMoney(
                          supplierRows.reduce((a, r) => a + r.current, 0),
                        )}
                      </td>
                      <td
                        style={{
                      ...
                      >
                        {fmtMoney(
                          supplierRows.reduce((a, r) => a + r.b1_30, 0),
                        )}
                      </td>
                      <td
                        style={{
                      ...
                      >
                        {fmtMoney(
                          supplierRows.reduce((a, r) => a + r.b31_60, 0),
                        )}
                      </td>
                      <td
                        style={{
                      ...
                      >
                        {fmtMoney(
                          supplierRows.reduce((a, r) => a + r.b61_90, 0),
                        )}
                      </td>
                      <td
                        style={{
                      ...
                      >
                        {fmtMoney(
                          supplierRows.reduce((a, r) => a + r.b90_plus, 0),
                        )}
                      </td>
                      <td
                        style={{
