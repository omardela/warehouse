...
export async function createSalesCreditNoteAction(
  ...
  try {
    await requirePermission(session, "sales.creditnotes.create");
  } catch {
    return { error: "You do not have permission to create sales credit notes." };
  }
  ...
  if (!invoice) return { error: "Sales invoice not found." };
  if (invoice.warehouseId !== session.warehouseId) return { error: "Access denied." };
  if (invoice.type !== "SALE") return { error: "Not a sales invoice." };
  if (invoice.status !== "CONFIRMED") {
    return { error: "Credit notes can only be raised against a confirmed sales invoice." };
  }
  ...
  await writeAuditLog({
    ...
    action: "sales.creditnotes.create",
    entityType: "CreditNote",
    entityId: creditNote.id,
    after: { type: "SALE", status: "DRAFT", originalInvoiceId: invoice.id, lines: nonZeroLines.length },
  });
...
// ─── Confirm Sales Credit Note ─────────────────────────────────────────────

export async function confirmSalesCreditNoteAction(
  _prevState: CreditNoteActionState,
  formData: FormData
): Promise<CreditNoteActionState> {
  const session = await getSession();
  ...
  if (!creditNoteId) return { error: "Credit note ID is required." };

  try {
    await requirePermission(session, "sales.creditnotes.create");
  } catch {
    return { error: "You do not have permission to confirm sales credit notes." };
  }
  ...
  if (creditNote.warehouseId !== session.warehouseId) return { error: "Access denied." };
  if (creditNote.type !== "SALE") return { error: "Not a sales credit note." };
  if (creditNote.status !== "DRAFT") return { error: "Only draft credit notes can be confirmed." };
  ...
  for (const line of creditNote.lines) {
    await recordMovement({
      ...
      notes: `Sales return against credit note`,
    });
  }

  await writeAuditLog({
    actorId: session.employeeId,
    action: "sales.creditnotes.confirm",
    entityType: "CreditNote",
  ...
  revalidatePath("/dashboard/sales/credit-notes");
  revalidatePath(`/dashboard/sales/credit-notes/${creditNoteId}`);
...
export async function cancelSalesCreditNoteAction(
  ...
  try {
    ...
    return { error: "You do not have permission to cancel sales credit notes." };
  }
  ...
  if (creditNote.status !== "DRAFT") {
    return { error: "Only draft credit notes can be cancelled; confirmed credit notes are immutable." };
  }
  ...
  await writeAuditLog({
    actorId: session.employeeId,
    action: "sales.creditnotes.cancel",
    entityType: "CreditNote",
    entityId: creditNoteId,
    before: { status: beforeStatus },
