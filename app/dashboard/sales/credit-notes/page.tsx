...
export default async function SalesCreditNotesPage({ searchParams }: PageProps) {
  ...
  const [creditNotes, total] = await Promise.all([
    db.creditNote.findMany({
      ...
      take: PAGE_SIZE,
      include: {
        originalInvoice: { select: { id: true, customer: { select: { id: true, name: true } } } },
      ...
      <div style={{ maxWidth: "1200px", margin: "0 auto" }}>
        ...
        >
          <div>
            ...
            <p style={{ fontSize: "13px", color: "#8c90a2", marginTop: "4px" }}>
              Returns raised against confirmed sales invoices.
            </p>
          </div>
        </div>
        ...
        >
          {["", "DRAFT", "CONFIRMED", "CANCELLED"].map((s) => (
            <Link
              key={s}
              href={s ? `?status=${s}` : "/dashboard/sales/credit-notes"}
              style={{
                padding: "6px 14px",
                borderRadius: "20px",
                fontSize: "12px",
            ...
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: "13px" }}>
              <thead>
                <tr style={{ borderBottom: "1px solid #222a3e", background: "#0d1627" }}>
                  {["Credit Note ID", "Customer", "Original Invoice", "Total Value", "Status", "Created", "Actions"].map((h) => (
                    <th
                      key={h}
                      style={{
                        padding: "12px 16px",
                ...
                {creditNotes.length === 0 ? (
                  <tr>
                    <td colSpan={7} style={{ padding: "48px 24px", textAlign: "center", color: "#8c90a2", fontSize: "14px" }}>
                      {statusFilter ? `No ${statusFilter.toLowerCase()} credit notes found.` : "No sales credit notes yet."}
                    </td>
                  </tr>
                ) : (
                  creditNotes.map((cn) => {
                    ...
                    return (
                      <tr key={cn.id} className="cn-row" style={{ borderBottom: "1px solid #1a2237" }}>
                        ...
                        <td style={{ padding: "12px 16px", color: "#dbe2fd" }}>
                          {cn.originalInvoice.customer?.name ?? "—"}
                        </td>
                        <td style={{ padding: "12px 16px" }}>
                          <Link
                            href={`/dashboard/sales/${cn.originalInvoice.id}`}
                        ...
                        <td style={{ padding: "12px 16px" }}>
                          <Link
                            href={`/dashboard/sales/credit-notes/${cn.id}`}
